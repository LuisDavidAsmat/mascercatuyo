...
interface ServiceOfferOwnerProps 
{
    rating: number;
...
const ServiceOfferOwner: React.FC<ServiceOfferOwnerProps> = ({ rating, showContactButtons, serviceId }) =>  {
  ...
  useEffect(() => {
    const fetchOwnerData = async () => {
      try {
        const serviceResponse = await apiClient.get<any>(`/services/offer/details/${serviceId}`);
        const ownerUserId = serviceResponse.data.userId;
        console.log(ownerUserId);
        
        setProviderId(ownerUserId); // Set providerId immediately
  ...
  return (
    <>
    {/* {providerId} */}
      {/* <Carousel userIdP={providerId}/> */}
      {providerId !== 0 ? ( // Or your initial "not loaded" value
      <Carousel userIdP={providerId} />
    ) : (
      <div>Cargando información del propietario...</div> 
    )}
